        ...
        string-tweak (fn [st]
                       (apply str (map (fn [c]
                                         (if (< (lrand) uniform-mutation-string-char-change-rate)
                                           ...
                                           c))
                                       st)))
        instruction-mutator (fn [token index]
                              (case genome-representation
                                     :plush (assoc token
                                                   :instruction
                                                   (:instruction
                                                    (random-plush-instruction-map atom-generators argmap index (count ind))))
                                     :plushy (random-plushy-instruction atom-generators argmap index (count ind))))
        constant-mutator (fn [token index]
                           (let [const (case genome-representation
                             ...
                             (case genome-representation
                               ...
                               :plushy new-const)))
        token-mutator (fn [token index]
                        (if (< (lrand) uniform-mutation-rate)
                          (if (< (lrand) uniform-mutation-constant-tweak-rate)
                            (constant-mutator token index)
                            (instruction-mutator token index))
                          token))
        new-genome (map-indexed (fn [index item] (token-mutator item index))(:genome ind))]
...
(defn uniform-addition
  ...
  (let [rate (random-element-or-identity-if-not-a-collection uniform-addition-rate)
        new-genome (vec (apply concat
                               (vec (map-indexed (fn [index item] (if (< (lrand) rate)
                                                                          (lshuffle [item
                                                                            (random-genome-gene
                                                                              atom-generators argmap index (count ind))])
                                                                          [item]))
                                                  (:genome ind)))
                               ; (mapv #(if (< (lrand) rate)
                               ;          (lshuffle [%
...
(defn uniform-addition-and-deletion
  ...
  (let [addition-rate (random-element-or-identity-if-not-a-collection uniform-addition-and-deletion-rate)
        ...
        after-addition (vec (apply concat
                                    (vec (map-indexed (fn [index item] (if (< (lrand) addition-rate)
                                                     (lshuffle [item
                                                       (random-genome-gene
                                                         atom-generators argmap index (count ind))])
                                                     [item]))
