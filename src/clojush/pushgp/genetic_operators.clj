...
(defn uniform-two-point-crossover
  "Returns child genome created through crossover of two parents. Each gene in parent1
  is considered in turn, and depending on some probability, a section of random lengths
  may be replaced by the corresponding section from parent 2. Probability is given by
  uniform-two-point-crossover-rate.
  Works with Plushy genomes."
  [parent1 parent2
   {:keys [uniform-two-point-crossover-rate maintain-ancestors]
    :as argmap}]
  (let [combination-rate (random-element-or-identity-if-not-a-collection uniform-two-point-crossover-rate)
        genome-1 (:genome parent1)
        genome-2 (:genome parent2)
        size (count genome-1)
        new-genome (vec (loop [index 0
                              genome []]
                        (if (>= index size)
                          genome
                          (if (< (lrand) combination-rate)
                          (let [rand (rand-int 10)
                                vec-start (if (< index (count genome-2))
                                              index
                                              (count genome-2))
                                vec-end (if (< (+ index rand)(count genome-2))
                                                (+ index rand)
                                                (count genome-2))]
                            (recur (+ index rand) (vec (concat genome (subvec genome-2 vec-start vec-end)))))
                          (recur (+ index 1) (vec (conj genome (nth genome-1 index))))))))]
    (make-individual :genome new-genome
