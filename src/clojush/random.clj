...
(defn quarter-calculator
 [index count]
 (if (= count 0) 0
   (let [percent (* (/ index count) 100)
       quarter (if (<= 0 percent 25) 0
                   (if (<= 26 percent 50) 1
                       (if (<= 51 percent 75) 2
                           (if (<= 76 percent 100) 3 0))))]
       quarter)))

(defn random-atom
  "Returns an atom generated by a randomly chosen atom-generator."
  [atom-generators {:keys [boosted-atom-generators boosted-atom-generators-probability]} index count]
  (let [quarter (quarter-calculator index count)
        element (if (nil? boosted-atom-generators)
                    (lrand-nth atom-generators)
                    (if (< (lrand) boosted-atom-generators-probability)
                        (lrand-nth (nth boosted-atom-generators quarter))
                        (lrand-nth atom-generators)))]
    (if (fn? element)
